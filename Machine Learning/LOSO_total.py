...
In this script the nested LOSO classification scheme for the grand average
case is implemented, as described in ( https://doi.org/10.36227/techrxiv.19589728.v1).
...
models = [SVC(),SVC(),KNeighborsClassifier(), LogisticRegression(),DecisionTreeClassifier()]#,RandomForestClassifier()]
kscore_func = f_classif

path_init = os.getcwd()
path_init = os.path.join(path_init, 'total_classification_results')
...
"Load Datasets"
rof   = pd.read_csv('Features_rof_new.csv')
r15of = pd.read_csv('Features_r15of_new.csv')
r30of = pd.read_csv('Features_r30of_new.csv')
r45of = pd.read_csv('Features_r45of_new.csv')
r60of = pd.read_csv('Features_r60of_new.csv')
...
r45of = r45of.to_numpy()
r60of = r60of.to_numpy()
...
"Get bspecEn128 & bspecEn228"
bspecEn128_with_rof = datasetDf['bspecEn12h8']
bspecEn228_with_rof = datasetDf['bspecEn22h8']



"Make folder"
f2name = os.path.join(path_init,'Bis')
if not os.path.exists(f2name):
    os.mkdir(f2name)
    
"Save bspecEn128 & bspecEn228"
if save_Bic and exists(os.path.join(f2name, 'bspecEn128_with_rof.csv')) == False:
    bspecEn128_with_rof.to_csv(os.path.join(f2name, 'bspecEn128_with_rof.csv'),index = False)
if save_Bic and exists(os.path.join(f2name, 'bspecEn228_with_rof.csv')) == False:
    bspecEn228_with_rof.to_csv(os.path.join(f2name, 'bspecEn228_with_rof.csv'),index = False)
    
    
"Get bicEn128 & bicEn228"
bicEn128_with_rof = datasetDf['bicEn12h8']
bicEn228_with_rof = datasetDf['bicEn22h8']
totalBic_with_rof = datasetDf['totalBic2h8']



"Make folder"
f2name = os.path.join(path_init,'Bic')
if not os.path.exists(f2name):
    os.mkdir(f2name)
    
"Save bicEn128 & bicEn228"
if save_Bic and exists(os.path.join(f2name, 'bicEn128_with_rof.csv')) == False:
    bicEn128_with_rof.to_csv(os.path.join(f2name, 'bicEn128_with_rof.csv'),index = False)
if save_Bic and exists(os.path.join(f2name, 'bicEn228_with_rof.csv')) == False:
    bicEn228_with_rof.to_csv(os.path.join(f2name, 'bicEn228_with_rof.csv'),index = False)
if save_Bic and exists(os.path.join(f2name, 'totalBic_with_rof.csv')) == False:
    totalBic_with_rof.to_csv(os.path.join(f2name, 'totalBic_with_rof.csv'),index = False)
